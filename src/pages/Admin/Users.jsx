...
import { FiPlus, FiUsers, FiFilter, FiUser } from 'react-icons/fi'
import { mockUsers, ROLES } from './usersMock'
...
export default function Users() {
  ...
  const counts = useMemo(() => {
    const admins = mockUsers.filter((u) => u.role === ROLES.ADMIN).length
    const cashiers = mockUsers.filter((u) => u.role === ROLES.CASHIER).length
    const frozen = mockUsers.filter((u) => u.isFrozen).length
    const active = mockUsers.filter((u) => !u.isFrozen).length
    return {
